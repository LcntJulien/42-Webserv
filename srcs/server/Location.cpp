...
/*   Updated: 2024/09/11 15:27:40 by jlecorne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void        Location::clear_inheritance(Server *cluster, std::string req_path) {
    ...
    if (file_exists(this->_cgi_path + path))
        return;
    ...
    if (path[path.length() - 1] == '/') {
        ...
        if (ppos != std::string::npos || !is_directory(this, path))
            path = path.substr(0, rpos + 1);
    ...
    if (this->get_autoindex() == true && ((tmp != NULL && tmp->get_autoindex() == false) || tmp == NULL))
