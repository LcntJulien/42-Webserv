...
/*   Updated: 2024/09/12 15:09:23 by jlecorne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    if (file_exists(path)) {
        if (access(path.c_str(), W_OK) == 0) {
            if (remove(path.c_str()) == 0) {
                std::cout << "errno: " << errno << std::endl; // debug
                res->req_return(config, "204");
            } else
                res->req_return(config, "500");
        } else
